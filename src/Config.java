...
public class Config {
	public static String rotate = "Up", left = "Left", 
			right = "Right", down = "Down", pause = "P";
	...
	public static void loadConfig() throws Exception{
		File directory = new File(getDefaultDirectory(), "/Memes");
		if(!directory.exists()) {
			directory.mkdirs();
		}
		...
		if(!config.exists()) {
			...
			System.out.println("Where's your file mate? I can't seem to find it.");
			saveConfig();
			return;
		}
		...
		if(values.size() != 5) {
			System.out.println("Config is getting reset because you done screwed up.");
			saveConfig();
			s.close();
			return;
		}
		if(!values.containsKey("left") || !values.containsKey("right")
				|| !values.containsKey("rotate") || !values.containsKey("down")
				|| !values.containsKey("pause")) {
			System.out.println("Err 404 Keys not found.");
			saveConfig();
			s.close();
			return;
		}
		...
		if(!(getKeyNames().contains(left) && getKeyNames().contains(right)
				&& getKeyNames().contains(rotate) && getKeyNames().contains(down)
				&& getKeyNames().contains(pause))) {
			System.out.println("Stop trying to use the wrong keys you dunce!");
			s.close();
			return;
		}
		Config.left = left;
